...
\begin{description}
    \item[2.0.0 (2024-??-??)] Major (breaking API) update: add AES-192, AES-256 and decryption \todo{FIXME}.
...
\todo{Revise, not real, must be at least 2 cycles between valid and ready}
...
\begin{itemize}
    ...
    \item Starting a reseeding takes precedence over starting a new key configuration. Therefore, if configuring a new key is required, no new 
        seed valid input should be asserted before the first key configuration transaction occured.
\end{itemize}
...
\todo{HERE Modify for arbitrer for valid ready signal}
...
\todo{Description of the key core}
...
\todo{Update with fresh synthesis results}
